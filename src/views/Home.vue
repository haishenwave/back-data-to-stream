  ...
  <div class="w-full h-screen bg-[#0b0f19] flex overflow-auto min-w-[1024px] min-h-[600px] text-gray-300 font-sans">
    ...
    <aside class="w-64 bg-[#111625] border-r border-white/5 flex flex-col flex-shrink-0 sticky top-0 h-screen z-10">
      ...
      <div class="p-4 border-t border-white/5 flex-shrink-0">
        <div class="flex items-center">
          ...
          <div class="ml-3">
            <p class="text-xs font-medium text-white">助农小分队</p>
    ...
    <main class="flex-1 flex flex-col bg-[#0b0f19] min-w-0">
      ...
      <header class="h-16 flex items-center justify-between px-8 bg-[#0b0f19] border-b border-white/5 flex-shrink-0 sticky top-0 z-10">
        <h2 class="text-lg font-medium text-white">数据周报概览</h2> <div class="flex items-center space-x-4">
        <div class="relative">
          ...
          <input type="text" placeholder="搜索周报/周期..." class="bg-[#161b2c] border border-gray-700 text-xs text-white rounded-full pl-10 pr-4 py-1.5 focus:outline-none focus:border-blue-500 w-64 transition-all focus:w-72">
        </div>
      ...
      <div class="flex-1 overflow-y-auto p-8">
        ...
        <div class="bg-[#161b2c] rounded-lg border border-white/5 overflow-hidden">
          <div class="px-6 py-4 border-b border-white/5 flex justify-between items-center">
            <h3 class="text-sm font-bold text-white">近期周数据归档 (3)</h3>
          ...
          <table class="w-full text-left text-xs">
            ...
            <tr>
              <th class="px-6 py-3 font-medium">数据报告主题</th>
              <th class="px-6 py-3 font-medium">统计周期</th>
              <th class="px-6 py-3 font-medium text-right">总GMV (元)</th>
              <th class="px-6 py-3 font-medium text-right">总订单数</th>
              <th class="px-6 py-3 font-medium text-center">状态</th>
              <th class="px-6 py-3 font-medium text-center">操作</th>
            </tr>
            </thead>
            <tbody class="divide-y divide-white/5">
            <tr v-for="item in liveRecords" :key="item.id" class="hover:bg-white/5 transition-colors group">
              <td class="px-6 py-4">
                <div class="flex items-center">
                  <div class="w-8 h-8 rounded bg-gray-700 flex-shrink-0 mr-3 overflow-hidden">
                    <div class="w-full h-full bg-blue-500/20 flex items-center justify-center text-[8px] text-blue-300">REPORT</div>
                  </div>
                  <span class="text-gray-200 font-medium">{{ item.title }}</span>
                </div>
              </td>
              <td class="px-6 py-4 text-gray-400 font-mono">{{ item.date }}</td>
              <td class="px-6 py-4 text-right text-white font-medium">{{ item.gmv }}</td>
              <td class="px-6 py-4 text-right text-gray-400">{{ item.orders }}</td>
              <td class="px-6 py-4 text-center">
                <span class="px-2 py-0.5 rounded-full text-[10px] bg-gray-700 text-gray-300">已归档</span>
              </td>
              <td class="px-6 py-4 text-center">
                <button
                    @click="goToDashboard"
...
// 注入真实业务数据 (同步 Jun 15 - Jun 21 涨幅)
const stats = [
  { title: '近7日累计 GMV', value: '1.86w', trend: 12.5 },
  { title: '近7日交易件数', value: '980', trend: 32.0 },
  { title: '近7日商品访客', value: '1,350', trend: 46.3 },
  { title: '综合转化率', value: '46.4%', trend: 15.6 },
];

// 历史数据模拟（按周为单位推演）
const liveRecords = [
  { id: 1, title: '6月第3周：助农数据总复盘', date: '2025.06.15 - 06.21', gmv: '18,652', orders: '980' },
  { id: 2, title: '6月第2周：助农数据总复盘', date: '2025.06.08 - 06.14', gmv: '16,580', orders: '742' },
  { id: 3, title: '6月第1周：助农数据总复盘', date: '2025.06.01 - 06.07', gmv: '14,200', orders: '610' },
];
